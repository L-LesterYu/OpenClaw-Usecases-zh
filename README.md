...
| OpenClaw桌面协作站 | OpenClaw 作为桌面协作者（AionUi）——远程救援与多智能体中心，通过桌面协作用户界面使用 OpenClaw，支持远程救援和多智能体集成 | usecases/OpenClaw桌面协作站/README.md |
...
| [AI视频编辑](usecases/AI视频编辑/ai-video-editing.zh.md) | 通过自然语言对话进行视频编辑，支持片段修剪、添加背景音乐、生成字幕、调色和批量处理，无需复杂的时间线编辑器 | 📦 | ⭐⭐ |
...
| [CI不稳定测试医生](usecases/CI不稳定测试医生/README.md) | 定期检测 CI 不稳定测试并构建修复队列，按失败签名聚类分析识别间歇性故障，创建优先级修复任务并推送摘要报告 | 🔔 📦 | ⭐⭐ |
...
- WordPress B2B外贸站点部署 → 🇨🇳 可替代为：**阿里云/腾讯云** VPS 替代国外 VPS，**又拍云/七牛** 替代 Cloudflare CDN，**宝塔面板**作为图形化管理补充，**微信/飞书/钉钉** 推送部署报告
...
| [每周研究摘要](usecases/每周研究摘要/README.md) | 通过网络搜索创建高信号每周摘要，按信号强度和新颖度排序发现，附带趋势洞察和后续行动建议 | 🔔 📦 | ⭐⭐ |
...
- AI视频剪辑 → 🇨🇳 通用性强，剪辑逻辑无地域限制，可配合国内视频平台使用
